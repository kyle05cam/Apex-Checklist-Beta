...
return (
                ...
                return (
                  <div>
                    ...
                    <div style={{ border: `1px solid ${ref.color}30`, borderRadius: 5, overflow: "hidden" }}>
                     ...
                     {row.map((cell, ci) => {
  // Dynamic color for high readability in both theme modes
  let cellColor;
  if (lightMode) {
    cellColor = "#050a15"; // jet black default for all cells in light mode
    const lower = cell.toLowerCase();
    if (lower.includes("green")) cellColor = "#0b532b";
    if (lower.includes("red"))   cellColor = "#a01005";
    if (lower.includes("yellow") || lower.includes("amber")) cellColor = "#7a5800";
  } else {
    cellColor = ci === 0 ? "#e8e4d8" : ref.color;
  }
  ...
  return (
    <div key={ci} style={{
      ...
      padding:    "9px 12px",
      lineHeight: 1.3,
      borderRight: ci < ref.cols.length - 1
        ? `1px solid ${lightMode ? "rgba(0,0,0,0.1)" : `${ref.color}12`}`
        : "none",
    }}>
      {cell}
    </div>
